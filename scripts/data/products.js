...
export function loadProducts(functionToRun) {
  const httpRequestForProducts = new XMLHttpRequest();

  httpRequestForProducts.addEventListener('load', () => {
    products = JSON.parse(httpRequestForProducts.response).map((product) => {
      if (product.type === "clothing")
        return new Clothing(product);
      else if (product.type === "appliance")
        return new Appliance(product);
  ...
  httpRequestForProducts.open('GET', 'https://supersimplebackend.dev/products');
  httpRequestForProducts.send();
}
